...
def all_cols(thesaurus_c):
    _, cols, _ = thesaurus_c.to_sparse_matrix(row_transform=None, column_transform=None)
    assert len(cols) == 5
    return cols
...
def test_filter_out_infrequent_entries(thesaurus_c, all_cols, feature_type_limits, expected_shape, missing_columns):
    ...
    assert set(all_cols) - set(DocumentFeature.from_string(x) for x in missing_columns) == set(cols)
